...
cond grasped(obj:object)
    ...
    expect(parser.file(0), AstFile(vec![
        ...
        FileEntity::Tree(Tree::new(TreeType::Root, "ball".to_string(),
        ...
        FileEntity::Tree(Tree::new(TreeType::Sequence, "try_to_place_to".to_string(),
                                   Params::new(vec![Param::new("obj", MesType::Object)]), Calls::new(vec![
                Call::lambda(TreeType::Fallback, Calls::new(vec![
                    Call::invocation("find_ball", Arguments::new(vec![Argument::id("obj")]))
                ]))
            ]))),
        FileEntity::Tree(Tree::new(TreeType::Cond, "grasped".to_string(),
...
cond ball_found {}
    ...
    expect(parser.file(0), AstFile(vec![
        FileEntity::Tree(Tree::new(
            TreeType::Cond,
            "grasped".to_string(), Params::default(),
            Calls::default()
        )),
        FileEntity::Tree(Tree::new(
            TreeType::Cond,
            "ball_found".to_string(), Params::default(),
            Calls::default()
        ))
    ]));
}
