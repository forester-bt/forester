...
use crate::tree::parser::ast::{AstFile, FileEntity, Key, Import, ImportName, Tree};
...
impl<'a> Project {
    pub fn find_tree(&self, file: &FileName, tree: &TreeName) -> Option<&Tree> {
        self
            .files
            .get(file)
            .and_then(|f| f.definitions.get(tree))
    }
    ...
    pub fn build(main_file: FileName, root: PathBuf) -> Result<Project, TreeError> {
        ...
        let main_call =
            project
                ...
                .ok_or(TreeError::IOError(format!("no root operation in the file {}", main_file.clone())))?;
