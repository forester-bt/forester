...
use dashmap::DashMap;
use forester_language_server::chumsky::{parse, type_inference, Func, ImCompleteSemanticToken};
use forester_language_server::completion::completion;
use forester_language_server::jump_definition::get_definition;
use forester_language_server::reference::get_reference;
use forester_language_server::semantic_token::{semantic_token_from_ast, LEGEND_TYPE};
use ropey::Rope;
        ...
        let inlay_hint_list = hashmap
            ...
            .map(|(k, v)| {
                (
                    ...
                    k.end,
                    match v {
                        forester_language_server::chumsky::Value::Null => "null".to_string(),
                        forester_language_server::chumsky::Value::Bool(_) => "bool".to_string(),
                        forester_language_server::chumsky::Value::Num(_) => "number".to_string(),
                        forester_language_server::chumsky::Value::Str(_) => "string".to_string(),
                        forester_language_server::chumsky::Value::List(_) => "[]".to_string(),
                        forester_language_server::chumsky::Value::Func(_) => v.to_string(),
                    },
                )
            })
            .filter_map(|item| {
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let completions = || -> Option<Vec<CompletionItem>> {
            ...
            for (_, item) in completions {
                match item {
                    forester_language_server::completion::ImCompleteCompletionItem::Variable(var) => {
                    ...
                    forester_language_server::completion::ImCompleteCompletionItem::Function(
                        name,
                        args,
                    ) => {
                        ret.push(CompletionItem {
